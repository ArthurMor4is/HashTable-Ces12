...
public:
     ...
     * \return number of collisions: how many strings were acessed to determine if the string was aready in the hashtable
     */
    int add(std::string str) ;
    
    /**
     * \brief the size of the largest list in the hashtable, i.e, the worst case for a search.
     * \par this is obviously O(n), as it has to check every list. It is not usual, it is here  only for didactical purposes.
     */
    int worst_case();
...
private:
    ...
    /// nice teacher already tells you which stl class to use
